...
bool TypeChecker::visit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::TypeType:
	{
		...
		case Type::Category::Contract:
		{
			...
			if (auto const* functionTypeMember = dynamic_cast<FunctionType const*>(accessedMemberAnnotation.type))
			{
				...
				else if (functionTypeMember->kind() == FunctionType::Kind::Internal)
				{
					...
					if (
						...
						accessedMemberAnnotation.isPure = variableDeclarationMember->isConstant();
					else if (dynamic_cast<FunctionDefinition const*>(accessedMemberAnnotation.referencedDeclaration))
						accessedMemberAnnotation.isPure = true;
					else
				...
				else if (functionTypeMember->kind() == FunctionType::Kind::External)
				{
					...
					if (
						...
						solAssert(
							dynamic_cast<FunctionDefinition const*>(accessedMemberAnnotation.referencedDeclaration),
			...
			else if (auto const* typeTypeMember = dynamic_cast<TypeType const*>(accessedMemberAnnotation.type))
			{
				solAssert(
			...
			else if (auto const* varDecl = dynamic_cast<VariableDeclaration const*>(accessedMemberAnnotation.referencedDeclaration))
				accessedMemberAnnotation.isPure = varDecl->isConstant();
			else
	...
	case Type::Category::Contract:
	{
		...
		// Below only the sanity checks.
		if (dynamic_cast<FunctionDefinition const*>(accessedMemberAnnotation.referencedDeclaration))
		{
			auto const* accessedMemberFunctionType = static_cast<FunctionType const*>(accessedMemberAnnotation.type);
		...
		else if (dynamic_cast<VariableDeclaration const*>(accessedMemberAnnotation.referencedDeclaration))
		{
